# -*- coding: utf-8 -*-
"""
Created on Wed Feb 13 11:26:53 2019
...
Zold[1:-1,1:-1] = (1 - (sigma * tau**2 / mu)*((1/(hx**2)) + (1/(hy**2)))) * Z[1:-1,1:-1] + ((sigma * tau**2)/(2 * mu * hx**2)) * (Z[1:-1,2:] + Z[1:-1,:-2]) + ((sigma * tau**2)/(2 * mu * hy**2)) * (Z[2:,1:-1] + Z[:-2,1:-1]) - tau * np.exp(-5 * (X[1:-1,1:-1]**2 + Y[1:-1,1:-1]**2))
